...
const PendingTasks: React.FC = () => {
  ...
  const fetchTasks = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:5000/api/tasks/pending?${params}`);
  ...
  const handleDownload = async (attachment: Attachment) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/uploads/${attachment.filename}`);
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-4 gap-6">
        {currentTasks.map((task) => (
          <div
            key={task._id}
            className={`group rounded-xl shadow-sm border hover:shadow-lg transition-all duration-300 overflow-hidden transform hover:-translate-y-1 ${
          ...
          <table className="min-w-full divide-y divide-[--color-border]">
            ...
            <tbody className="divide-y divide-[--color-border]">
              {currentTasks.map((task, index) => (
                <tr
                  key={task._id}
                  className={`transition-all duration-200 hover:bg-[--color-surface] ${
                    theme === 'light'
