...
import { RefreshCw, Clock, Calendar, CheckSquare, Filter, Search, Users, ChevronDown, ChevronUp, AlertCircle, CalendarDays, Paperclip, FileText, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight } from 'lucide-react';
...
const PendingRecurringTasks: React.FC = () => {
  ...
  const fetchTasks = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:5000/api/tasks/pending-recurring?${params}`);
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-4 gap-6">
      {currentTasks.map((task) => {
        ...
        return (
          <div
            key={task._id}
            className={`bg-[var(--color-background)] rounded-xl shadow-sm border hover:shadow-lg transition-all duration-300 overflow-hidden transform hover:-translate-y-1 ${
              overdue 
              ...
              return (
                <tr 
                  ...
                  className={`hover:bg-[var(--color-surface)] transition-colors ${
                    overdue ? 'bg-[var(--color-error)]/10 hover:bg-[var(--color-error)]/20' 
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-[var(--color-text)]">{new Date(task.dueDate).toLocaleDateString()}</div>
                    {task.lastCompletedDate && (
        ...
        <div className="flex flex-wrap gap-2 mb-1 items-center">
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className={`px-4 py-1 rounded-lg font-semibold transition-all duration-200 flex items-center gap-2 text-xs ${
                activeSection === 'daily'
            ...
            >
              ...
              {dailyTasksCount > 0 && (
                <span className={`px-1 py-1 rounded-full text-xs font-bold ${
                  activeSection === 'daily' ? 'bg-[var(--color-background)] text-[var(--color-primary)]' : 'bg-[var(--color-primary)]/10 text-[var(--color-primary)]'
                }`}>
                  {dailyTasksCount}
                </span>
              )}
            ...
            >
              ...
              {cyclicTasksCount > 0 && (
                <span className={`px-2 py-1 rounded-full text-xs font-bold ${
                  activeSection === 'cyclic' ? 'bg-white text-[var(--color-accent)]' : 'bg-[var(--color-accent)]/10 text-[var(--color-accent)]'
                }`}>
                  {cyclicTasksCount}
                </span>
              )}
                        ...
                        {isImage(attachment.filename) ? (
                          <>
                            ...
                            <img
                              ...
                              onClick={() => setSelectedImagePreview(`http://localhost:5000/uploads/${attachment.filename}`)} // Set for full screen
                            />
